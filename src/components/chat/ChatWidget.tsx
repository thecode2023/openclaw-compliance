                    ...
                    {messages.map((msg, msgIndex) => {
                      const formatForShare = () => {
                        const sources = msg.citations?.length
                          ? `\n\nSources:\n${msg.citations.map((c) => `- ${c.title} — ${c.jurisdiction}`).join("\n")}`
                          : "";
                        return `--- Complyze AI Analysis ---\n\n${msg.content}${sources}\n\nGenerated by Complyze — complyze.dev\n${new Date().toLocaleDateString()}`;
                      };
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`max-w-[85%] rounded-lg px-3 py-2 text-sm group/msg relative ${
                              msg.role === "user"
                              ...
                              <>
                                ...
                                {msg.content && !isStreaming && (
                                  <div className="absolute -top-1 right-1 flex gap-0.5 opacity-0 group-hover/msg:opacity-100 transition-opacity">
                                    <button
                                      onClick={() => {
                                        navigator.clipboard.writeText(formatForShare());
                                        toast.success("Copied to clipboard");
                                      }}
                                      className="p-1 rounded bg-card/80 text-muted-foreground hover:text-foreground transition-colors"
                                      title="Copy answer"
                                    >
                                      <Clipboard className="w-3 h-3" />
                                    </button>
                                    <a
                                      href={`mailto:?subject=${encodeURIComponent(`Complyze Analysis: ${prevUserMsg?.content.slice(0, 50) || "Regulatory Question"}`)}&body=${encodeURIComponent(formatForShare())}`}
                                      className="p-1 rounded bg-card/80 text-muted-foreground hover:text-foreground transition-colors"
